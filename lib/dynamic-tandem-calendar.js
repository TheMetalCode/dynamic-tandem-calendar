...
function setIframe (subdomain, searchInput, schoolFilterNum, viewType, elemEmbedId) {
  ...
  const iframeSrc = 'http://' + subdomain + '.tandemcal.com/index.php?search_input=' +
  ...
  const iframeStyles = 'width: 1000px; height: 800px; border: 0; -ms-transform: scale(0.8);' +
  ...
  const iframeHtml = '<iframe style="' + iframeStyles + '" src="' +
    iframeSrc + '"></iframe>';
